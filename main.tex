...
\subsection{The most Sigma commentator (density)}
	\begin{table}[H]
		\centering
		\caption{Top-20 authors with most likes. Full table here \cite{sheet-count-likes}.}
		\label{table-count-likes}
		\includegraphics[width=0.5\textwidth]{table-count-likes-488}
	\end{table}

	The number of likes and the number of comments certainly does not give us all the information. Possibly there are sigma's who keep silent, but when they speak something, it caries a lot of value.

	How let us look at the distribution of the number of comments and likes.


	Here Физтех.Confessions and Василий Андрианов were excluded to keep the scales reasonable of axes reasonable. We can see that the number of likes and the number of comments deviates significantly from the trend line.
